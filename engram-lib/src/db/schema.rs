...
pub async fn create_tables(conn: &Connection) -> Result<()> {
    // -------------------------------------------------------------------------
    // Core tables (no vector indexes -- those need individual execute() calls)
    // -------------------------------------------------------------------------
    conn.execute_batch(
        "
        -- Users
    ...
    // -------------------------------------------------------------------------
    // FTS virtual tables -- separate execute() calls
    // -------------------------------------------------------------------------
    ...
    // -------------------------------------------------------------------------
    // Cross-tenant link prevention trigger
    // -------------------------------------------------------------------------
    ...
    conn.execute(
        "CREATE INDEX IF NOT EXISTS skill_records_vec_1024_idx ON skill_records(libsql_vector_idx(embedding_vec_1024))",
        (),
    )
    .await?;
