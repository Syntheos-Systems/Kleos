...
pub async fn detect_communities(db: &Database, user_id: i64, max_iterations: u32) -> Result<CommunitiesResult> {
    ...
    if m == 0.0 {
        for (idx, &id) in memory_ids.iter().enumerate() {
            conn.execute("UPDATE memories SET community_id = ?1 WHERE id = ?2 AND user_id = ?3", libsql::params![idx as i64, id, user_id]).await?;
        }
    ...
    for (&node_id, &comm) in &community {
        ...
        conn.execute("UPDATE memories SET community_id = ?1 WHERE id = ?2 AND user_id = ?3", libsql::params![cid, node_id, user_id]).await?;
    }
