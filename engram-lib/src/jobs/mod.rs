...
pub async fn claim_next_job(conn: &Connection) -> Result<Option<Job>> {
    ...
    Ok(Some(Job { id, job_type: jt, payload: pl, status: JobStatus::Running, attempts: att + 1, max_attempts: ma, error: None, created_at, claimed_at: Some(chrono::Utc::now().format("%Y-%m-%d %H:%M:%S").to_string()), completed_at: None, next_retry_at }))
}
...
pub async fn count_failed_jobs(conn: &Connection) -> Result<i64> {
    let mut rows = conn.query("SELECT COUNT(*) FROM jobs WHERE status = 'failed'", ()).await?;
